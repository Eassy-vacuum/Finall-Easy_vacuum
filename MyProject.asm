...
;MyProject.mbas,144 :: 		if ((oldstate_menu mod 6)=1) or ((oldstate_menu mod 6)=3)or ((oldstate_menu mod 6)=5) then
	...
	CPI        R16, 1
	LDI        R18, 0
	BRNE       L__key_test_old147
	LDI        R18, 255
L__key_test_old147:
	CPI        R16, 3
	LDI        R17, 0
	BRNE       L__key_test_old148
	LDI        R17, 255
L__key_test_old148:
	OR         R17, R18
	CPI        R16, 5
...
;MyProject.mbas,159 :: 		or (Button(Ok_SW_Port,Ok_SW_bit,50,1)<> 0)then    ' Detect logical one setting key
	...
	CPI        R16, 0
	LDI        R17, 0
	BREQ       L__key_test_old153
	LDI        R17, 255
L__key_test_old153:
	LDD        R16, Y+25
	OR         R16, R17
	BRNE       L__key_test_old154
...
;MyProject.mbas,187 :: 		or (Button(Ok_SW_Port,Ok_SW_bit,200,1)<> 0)then    ' Detect logical one setting key
	...
	CPI        R16, 0
	LDI        R17, 0
	BREQ       L__key_test_old160
	LDI        R17, 255
L__key_test_old160:
	LDD        R16, Y+25
	OR         R16, R17
	BRNE       L__key_test_old161
...
;MyProject.mbas,391 :: 		if (oldstate_4 and Button(Menu_SW_Port,Menu_SW_bit,2,0)) then
	...
	JMP        L__key_test106
L__key_test177:
;MyProject.mbas,413 :: 		if (seconds< k +2 )  then
	LDS        R16, _k+0
	LDS        R17, _k+1
	MOVW       R18, R16
	SUBI       R18, 254
